...
public class PenelopeF { // executable main class
    ...
    public static void createNewTestData(RepositoryManager repositories) { // Ugly, awkward test method to create users, groups etc. and test serialisation. Run, stop, run again, observe
        ...
        while (creating) {
            ...
            if (com.penelopef.tools.ScannerTools.scanString().toUpperCase().equals("Y"))
                os.createUser();
            else creating = false;
        }
        ...
        while (creating) {
            ...
            if (com.penelopef.tools.ScannerTools.scanString().toUpperCase().equals("Y")) {
                ...
                Group group;
                while ((group = com.penelopef.tools.DataTools.getGroupFromName(com.penelopef.tools.ScannerTools.scanString())) == null)
                ...
                for (UUID userId : testProject.getGroup().getUsersIds()) {
                    com.penelopef.tools.DataTools.getUserFromId(userId).getProjectsIds().add(testProject.getId());
                }
